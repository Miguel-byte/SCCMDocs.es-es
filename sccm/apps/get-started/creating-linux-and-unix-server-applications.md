---
title: Crear aplicaciones de servidor Linux y UNIX | System Center Configuration Manager
...
Además de los otros requisitos y procedimientos de System Center Configuration Manager para crear una aplicación, también debe tener en cuenta las consideraciones siguientes al crear e implementar aplicaciones para equipos que ejecutan Linux y UNIX.  
...
-   Puede instalar software para servidores Linux y UNIX, incluyendo lo siguiente:  

    -   Nueva implementación de software  

    -   Actualizaciones de software para los programas ya existentes en el equipo  

    -   Revisiones del sistema operativo  

-   Puede ejecutar comandos nativos de Linux y UNIX y ejecutar scripts que se encuentran en servidores Linux y UNIX.  

-   Puede limitar la implementación a los sistemas operativos que especifique cuando selecciona la opción de programa **Solo en plataformas cliente especificadas**.  

-   Puede utilizar ventanas de mantenimiento para controlar el momento en que se instala el software.  

-   Puede utilizar mensajes de estado de implementación para supervisar las implementaciones.  

-   El cliente podrá limitar el uso de la red al descargar el software desde un punto de distribución.  

 Las principales diferencias entre implementar paquetes y programas en equipos Linux y UNIX e implementar paquetes y programas en dispositivos Windows son:  
...
-   No se admiten las configuraciones que interactúan con los usuarios  
...
|Cuando este programa se asigne a un equipo:<br /><br /> - Ejecutar una vez para cada usuario que inicie sesión|Se omite la configuración|No se admiten las opciones de configuración específicas del usuario.<br /><br /> Sin embargo, el cliente sí es compatible con la configuración que especifica ejecutar una vez para el equipo.<br /><br /> Esta configuración no genera un error ni una entrada de registro porque ya se crearon un error y una entrada de registro para la configuración de requisitos previos relacionados con **El programa se puede ejecutar** = **Solo cuando un usuario haya iniciado sesión**.|  
...
|Permitir que este programa se instale desde la secuencia de tareas de instalación de paquete sin implementarse.|Se omite la configuración|El cliente no admite secuencias de tareas.<br /><br /> Esta configuración no es compatible y no afecta a la instalación de software.|  
...
### <a name="deployments"></a>Implementaciones  
 Para implementar software en un servidor Linux o UNIX mediante un paquete y un programa, puede usar el **Asistente para implementar software** de la consola de Configuration Manager. El cliente admite la mayoría de las opciones de configuración de implementación para Linux y UNIX; sin embargo, existen varias opciones de configuración que no se admiten. Al implementar software, considere lo siguiente:  
...
-   **Permitir descargas de BITS fuera del período de limitación**  
 ...
 Si se interrumpe la descarga del software al cliente desde un punto de distribución, el cliente para Linux y UNIX no reanuda la descarga y en su lugar reinicia la descarga de todos el paquete de software.  

##  <a name="operations-for-software-deployments"></a>Operaciones para las implementaciones de software  
