...
-   Identificar superredes asignadas a un sitio de Active Directory y convertir cada superred en un límite de intervalo de direcciones IP.  

-   Publicar en Active Directory Domain Services (AD DS) de un bosque cuando está habilitada la publicación en ese bosque y la cuenta de bosque de Active Directory especificada tenga permisos en ese bosque.  
...
> [!CAUTION]  
>  Al configurar un ámbito de detección, seleccione solo los grupos que necesita detectar. Esto se debe a que la detección de grupos de Active Directory intenta detectar todos los miembros de cada grupo del ámbito de detección. La detección de grupos grandes puede requerir un uso considerable de ancho de banda y de recursos de Active Directory.  
...
De forma predeterminada, este método detecta información básica sobre el equipo, como la siguiente:  
...
En el cuadro de diálogo **Propiedades de detección de sistemas de Active Directory**, en la pestaña **Atributos de Active Directory**, puede ver la lista completa de atributos de objeto predeterminados que ha encontrado la detección de sistemas de Active Directory. También puede configurar el método para detectar atributos adicionales (extendidos).  
...
Use este método de detección para buscar en Active Directory Domain Services (AD DS) e identificar cuentas de usuario y atributos asociados. De forma predeterminada, este método detecta información básica sobre la cuenta de usuario, como lo siguiente:  
...
En el cuadro de diálogo **Propiedades de detección de usuarios de Active Directory**, en la pestaña **Atributos de Active Directory**, puede ver la lista completa de atributos de objeto predeterminados que ha encontrado la detección de usuarios de Active Directory. También puede configurar el método para detectar atributos adicionales (extendidos).
...
La detección de latidos difiere de otros métodos de detección de Configuration Manager. Está habilitada de forma predeterminada y se ejecuta en cada equipo cliente (en lugar de ejecutarse en un servidor de sitio) para crear un DDR. En el caso de los clientes de dispositivos móviles, el DDR lo creará el punto de administración usado por el cliente del dispositivo móvil. Para mantener el registro de base de datos de los clientes de Configuration Manager, no deshabilite la detección de latidos. Además de mantener el registro de base de datos, este método puede forzar la detección de un equipo como un nuevo registro de recursos o puede volver a llenar el registro de base de datos de un equipo que se eliminó de la base de datos.  
...
Use este método para detectar la topología de la red y para detectar dispositivos de la red que tengan una dirección IP. La detección de redes busca en la red recursos habilitados para IP con la consulta de servidores que ejecutan una implementación de Microsoft de DHCP, memorias caché del Protocolo de resolución de direcciones (ARP) en enrutadores, dispositivos habilitados para SNMP y dominios de Active Directory.  
...
-   **Dispositivo SNMP:** la detección de redes puede consultar directamente a un dispositivo SNMP. Para que la detección de redes consulte un dispositivo, este último debe tener instalado un agente SNMP local. También es necesario configurar la detección de redes para usar el nombre de la comunidad usado por el agente SNMP.  

Cuando la detección identifica un objeto accesible por IP y puede determinar la máscara de subred del objeto, crea un DDR para ese objeto. Debido a que pueden conectarse a la red diferentes tipos de dispositivos, la detección de redes puede detectar recursos que no son compatibles con el software cliente de Configuration Manager. Por ejemplo, entre los dispositivos que se pueden detectar, pero no administrar, se incluyen impresoras y enrutadores.  

La detección de redes puede devolver varios atributos como parte del registro de detección que se crea. Entre ellos, se incluye:  
...
-   Para detectar equipos del dominio local, es necesario habilitar el servicio Explorador de equipos como mínimo en un equipo que se encuentre en la misma subred que el servidor de sitio que ejecuta la detección de redes.  
...
-   Para detectar un recurso de equipo con la detección de sistemas de Active Directory es necesario que el agente de detección pueda resolver el FQDN del recurso. Si no puede resolver el FQDN, intentará resolver el recurso por su nombre NetBIOS.  
...
Configure el filtrado en la pestaña **Opción** de los cuadros de diálogo **Propiedades de detección de sistemas de Active Directory** y **Propiedades de detección de grupos de Active Directory**. Seleccione la opción **Detectar solo los equipos que iniciaron sesión en un dominio en un determinado período de tiempo**.  
...
Configure el filtrado en la pestaña **Opción** de los cuadros de diálogo **Propiedades de detección de sistemas de Active Directory** y **Propiedades de detección de grupos de Active Directory**. Seleccione la opción **Detectar solo los equipos que actualizaron su contraseña de cuenta de equipo en un determinado período de tiempo**.  
