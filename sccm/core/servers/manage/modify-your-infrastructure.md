...
El grupo de seguridad denominado **Administradores de SMS** concede acceso al proveedor de SMS desde un equipo y también puede usarse para conceder los permisos de DCOM necesarios. (Este grupo es local para el equipo cuando el proveedor de SMS se ejecuta en un servidor miembro, y es un grupo local de dominio cuando el proveedor de SMS se ejecuta en un controlador de dominio).  
