...
3.  En **Acceso a la aplicación**, en el caso de Outlook y las aplicaciones que usan la autenticación moderna, puede elegir restringir el acceso solo a los dispositivos compatibles con cada plataforma.  
    ...
    -   **Los dispositivos deben estar unidos a un dominio o ser compatibles**: Esto significa que los equipos deben estar unidos a un dominio o ser compatibles con las directivas establecidas en Intune. Si el equipo no cumple alguno de estos requisitos, se le solicita al usuario que inscriba el dispositivo en Intune.  

    -   **Los dispositivos deben estar unidos a un dominio**: Esto significa que los equipos deben estar unidos a un dominio para acceder a Exchange Online. Si el equipo no está unido a un dominio, el acceso al correo se bloquea y el usuario debe ponerse en contacto con el administrador de TI.  

    -   **Los dispositivos deben ser compatibles**: Esto significa que los equipos deben estar inscritos en Intune y ser compatibles. Si el equipo no está inscrito, se muestra un mensaje con instrucciones sobre cómo inscribirlo.  

4.  En **Acceso de explorador** a SharePoint Online y OneDrive for Business, se puede elegir permitir el acceso solo a Exchange Online a través de los exploradores admitidos: Safari (iOS) y Chrome (Android). Se bloquea el acceso desde otros exploradores. Las mismas restricciones de plataforma que seleccionó para el acceso a las aplicaciones para OneDrive también se aplica aquí.
    ...
    En las plataformas **iOS y Android**, para identificar el dispositivo que se usa para acceder al servicio, Azure AD emite un certificado de TLS para el dispositivo. El dispositivo muestra el certificado con un símbolo del sistema para que el usuario final seleccione el certificado tal y como se muestra en las siguientes capturas de pantalla: el usuario final debe seleccionar este certificado para poder seguir usando el explorador.
...
7.  En **Grupos destinatarios**, haga clic en **Modificar** para seleccionar los grupos de seguridad de Azure AD a los que se aplica la directiva.  

8.  Opcionalmente, en **Grupos exentos** puede hacer clic en **Modificar** para seleccionar los grupos de seguridad de Azure AD que se excluyen de esta directiva.  
