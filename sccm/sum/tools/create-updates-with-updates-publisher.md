...
2.  En la página **Paquete**, use la información siguiente como ayuda para configurar la actualización:
    ...
    -   Elija **Examinar** para buscar el paquete de actualización de software que va a usar como origen del paquete. Entre los orígenes válidos se incluyen archivos .MSI, .MSP o .EXE. Updates Publisher crea un hash del archivo. El nombre de archivo y de hash se usan después en los metadatos de actualización de la actualización que se va a crear.

    -   Especifique la ubicación de origen del contenido de esta actualización. Si tiene una copia local del contenido, puede activar la casilla **Use a local source to publish software update content** (Usar un origen local para publicar contenido de actualización de software) para utilizar la [ruta de acceso de publicación de origen local](/sccm/sum/tools/updates-publisher-options#advanced) (y la opción avanzada). Si esta opción no se selecciona, debe especificar una dirección URL donde se encuentre la actualización en Internet. Esta ruta de acceso o dirección URL se agrega a los metadatos de actualización.

        Más adelante, cuando la actualización se publique en un servidor WSUS, Updates Publisher obtiene los archivos binarios de la actualización de la ubicación de origen indicada.
 ...
 **Proveedor**: especifica un proveedor para actualización. Puede usar la lista desplegable para usar valores de actualizaciones que se encuentran en el repositorio. Cuando especifique un proveedor, el asistente crea una carpeta con el nombre del proveedor en **Todas las actualizaciones de software** en el **área de trabajo Actualizaciones** si esa carpeta no existe ya. Los siguientes son nombres reservados de Windows Server Update Services (WSUS) que no se pueden especificar para actualizaciones creadas por el usuario:
 ...
 -   Agrupación
 -   Actualización de agrupación  <br /><br />
...
1.  En la página **Información opcional**, puede configurar detalles que ofrezcan información adicional sobre la actualización.
...
2.  En la página **Requisitos previos**, especifique los requisitos previos que deben instalarse en un equipo para poder instalar esta actualización. Las reglas se denominan **detectoids**. Los detectoids son reglas de alto nivel como, por ejemplo, una que requiera que la CPU de los equipos sea un procesador de 64 bit. Los detectoids también pueden especificar actualizaciones que deben instalarse para poder instalar esta actualización.
 ...
 -   Al hacer clic en **Agregar grupo**, todos los detectoids que ha seleccionado se agregan a un solo grupo. Para optar a esta instalación, un equipo debe cumplir al menos uno de los requisitos previos de este grupo y pasar los requisitos de los grupos adicionales que están configurados.

1.  En la página **Sustitución**, especifique las actualizaciones que se reemplazan (sustituyen) por esta actualización. Cuando esta actualización se publique, Configuration Manager marcará cada actualización que se ha sustituido como **Caducada**. Los clientes instalarán luego esta actualización en lugar de las actualizaciones sustituidas.

2.  En la página **Aplicabilidad**, use el **Editor de reglas** para definir un conjunto de reglas que determinen si un dispositivo necesita esta actualización. (Esta página es similar a la página **Instalado** que la sigue).
...
1.  En la página **Instalado**, use el **Editor de reglas** para definir un conjunto de reglas que determinen si un dispositivo tiene ya instalada la actualización que está configurando. (Esta página es similar a la página **Aplicabilidad** que la precede).
...
Cuando el asistente termine, la nueva actualización se agrega a un nodo en el **área de trabajo Actualizaciones** que se identifica con el nombre del **Proveedor** que se use para la actualización.
